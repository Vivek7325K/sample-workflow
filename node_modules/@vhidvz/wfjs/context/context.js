...
class Context {
    ...
    resume(force = false) {
        if (force)
            this.status = enums_1.Status.Ready;
        else if (this.status !== enums_1.Status.Terminated) {
            this.status = enums_1.Status.Ready;
        }
        return this;
    }
     ...
     */
    isFailed() {
        return this.status === enums_1.Status.Failed;
    }
    /**
    ...
    isPartiallyTerminated() {
        return this.tokens.filter((t) => !t.locked).some((t) => t.status === enums_1.Status.Terminated);
    }
    /**
     ...
     */
    terminate() {
        this.status = enums_1.Status.Terminated;
        return this;
    }
    /**
    ...
    serialize({ data, value } = { data: true, value: true }) {
        return Object.assign(Object.assign({ status: this.status }, (data ? { data: this.data } : {})), { tokens: this.tokens.map((t) => t.serialize({ value })) });
    }
    /**
    ...
    static deserialize(ctx) {
        return new Context(Object.assign(Object.assign({}, ctx), { tokens: ctx.tokens.map((t) => token_1.Token.deserialize(t)) }));
    }
    /**
